...
// Widget StatefulWidget untuk halaman edit murid
// Halaman ini digunakan untuk mengedit data murid yang sudah ada di database
class EditMuridPage extends StatefulWidget {
  // Parameter wajib berupa ID murid yang akan diedit
  // ID ini digunakan untuk mengambil data murid dari database
  final int muridId;
  const EditMuridPage({Key? key, required this.muridId}) : super(key: key);

  @override
  _EditMuridPageState createState() => _EditMuridPageState();
}

class _EditMuridPageState extends State<EditMuridPage> {
  // Key untuk validasi form - digunakan untuk mengakses dan memvalidasi form
  final _formKey = GlobalKey<FormState>();
  // Flag untuk mengendalikan tampilan validasi - jika true, validasi akan dijalankan setiap kali user berinteraksi dengan form
  ...
  // Variabel untuk radio button tingkatan dan kelas serta checklist mata pelajaran
  ...
  void initState() {
    super.initState();
    // Memanggil method untuk memuat data murid saat widget pertama kali diinisialisasi
    _loadMuridData();
  }

  // Method untuk memuat data murid dari database berdasarkan widget.muridId
  // Menggunakan async-await karena operasi database bersifat asynchronous
  Future<void> _loadMuridData() async {
    try {
      // Mengambil data murid dari repository berdasarkan ID
      final murid = await _repo.getStudent(widget.muridId);
      // Memperbarui state dengan data murid yang diperoleh
      setState(() {
        // Mengisi controller dengan data dari database
        _nameController.text = murid['name'] ?? ''; // Nama murid
        _alamatController.text = murid['alamat'] ?? ''; // Alamat murid
        _noHpController.text = murid['no_hp'] ?? ''; // Nomor HP murid
        _tempatLahirController.text = murid['tempat_lahir'] ?? ''; // Tempat lahir murid
        _tanggalLahirController.text = murid['tanggal_lahir'] ?? ''; // Tanggal lahir murid
        _selectedTingkatan = murid['tingkatan']; // Tingkatan pendidikan
        _selectedKelas = murid['kelas']; // Kelas
        _sekolahController.text = murid['sekolah'] ?? ''; // Sekolah murid
        _namaOrtuController.text = murid['nama_orangtua'] ?? ''; // Nama orang tua/wali
        _noHpOrtuController.text = murid['no_hp_orangtua'] ?? ''; // Nomor HP orang tua/wali
        // Mengisi daftar mata pelajaran, memecah string menjadi list
        if (murid['mata_pelajaran'] != null &&
            murid['mata_pelajaran'].toString().isNotEmpty) {
          _selectedSubjects.clear(); // Hapus data sebelumnya
          _selectedSubjects.addAll(
            murid['mata_pelajaran']
                .toString()
                .split(',') // Memecah string berdasarkan koma
                .map((e) => e.trim()) // Menghapus spasi di awal dan akhir
                .toList(), // Mengkonversi ke list
          );
        }
      });
    } catch (e) {
  ...
  // Fungsi untuk menyimpan update data murid ke database
  // Menggunakan async-await karena operasi database bersifat asynchronous
  Future<void> _updateMurid() async {
    setState(() {
      _autoValidate = true; // Aktifkan validasi otomatis saat submit
    });
    
    // Validasi manual untuk memastikan tidak ada field yang terlewat
    ...
    if (!isComplete) {
      ...
      return; // Menghentikan proses update jika validasi gagal
    }
    
    // Jika validasi lolos, buat map data murid yang akan diperbarui ke database
    // Map ini berisi semua field yang diperlukan untuk tabel murid
    Map<String, dynamic> updatedMurid = {
      'id': widget.muridId, // ID murid yang akan diupdate
    ...
    try {
      ...
      await _repo.updateStudent(updatedMurid);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Gagal memperbarui data murid: $e'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Edit Data Murid')),
      body: SafeArea(
        child: Form(
          key: _formKey,
          ...
          child: ListView(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              // Radio button untuk Kelas (hanya tampil jika tingkatan sudah dipilih)
